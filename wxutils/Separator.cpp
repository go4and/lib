...
void addSeparator(wxGridBagSizer * psizer, int y, wxWindow * parent, const std::wstring & title)
{
    ...
    sizer->Add(new wxPanel(result, 0, 0, 32, 4, wxTAB_TRAVERSAL | wxBORDER_SUNKEN, title.c_str()), wxSizerFlags().Center());
    sizer->Add(new wxStaticText(result, wxID_ANY, title.c_str()), wxSizerFlags().Center().Border(wxALL & ~wxTOP, 4));
    sizer->Add(new wxPanel(result, 0, 0, 32, 4, wxTAB_TRAVERSAL | wxBORDER_SUNKEN, title.c_str()), wxSizerFlags().Center().Proportion(1));
